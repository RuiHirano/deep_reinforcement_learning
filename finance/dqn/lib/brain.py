...
class Brain(IBrain):
    ...
    def optimize(self):
        ...
        state_batch = torch.cat(list(batch.state), dim=1).to(device) # state: size(L, B, Hin) size(10,32,1)
        action_batch = torch.Tensor(list(batch.action)).unsqueeze(1).type(torch.int64).to(device) # action: size(32, 1) 
        ...
        # self.policy_net(state_batch) (10, 3)
    ...
    def decide_action(self, state): # state (L, B, H)
        #state = state.clone().detach().double()
